...
class FIRMSWebScraper:
    ...
    def fetch_thermal_data(self,
        ...
        if cache_key in self._cache:
            ...
            if (datetime.now(UTC) - ts).total_seconds() < self.cache_ttl:
        ...
        try:
            ...
            # Cache
            self._cache[cache_key] = (hotspots, datetime.now(UTC))
            ...
            for row in reader:
                try:
                    ...
                    if date_str and time_str:
                        ...
                        acq_date = datetime(year, month, day, hour, minute, tzinfo=UTC)
                    else:
                        acq_date = datetime.now(UTC)
...
# Singleton
_scraper: FIRMSWebScraper | None = None

def get_firms_scraper(api_key: str | None = None) -> FIRMSWebScraper:
