...
class SignalAttributor:
    ...
    Ensures that the removal of any single signal does not collapse the strategy.
    """
    
    def __init__(self):
        self.signal_types = ["port_throughput", "retail_footfall", "thermal_anomaly"]

    def compute_attribution(self, portfolio_returns: float, signal_weights: Dict[str, float]) -> Dict[str, AttributionResult]:
        """
        Naive linear attribution (Brinson-style) for initial reporting.
        In production: use LOO (Leave-One-Out) cross-validation for non-linear attribution.
        """
        results = {}
    ...
    def check_signal_dependency(self, results: Dict[str, AttributionResult]) -> bool:
        ...
        for stype, res in results.items():
            ...
            if contribution > 0.60:
                logger.warning(f"HIGH DEPENDENCY: Signal {stype} accounts for {contribution*100:.1f}% of P&L!")
                return False
        return True
...
class FacilityMapper:
    ...
    def __init__(self):
        # In production: Load from Bloomberg Supply Chain (SPLC) or FactSet Revere
        self._facility_to_ticker = {
            "facility_001": {"ticker": "MSFT", "weight": 0.02, "type": "data_center", "region": "Virginia, US"},
            "facility_002": {"ticker": "TSLA", "weight": 0.15, "type": "gigafactory", "region": "Berlin, DE"},
            "port_shanghai": {"ticker": "BABA", "weight": 0.08, "type": "export_hub", "region": "Shanghai, CN"},
            "port_rotterdam": {"ticker": "MAERSK", "weight": 0.12, "type": "maritime_hub", "region": "Rotterdam, NL"},
            "retail_mall_ca": {"ticker": "WMT", "weight": 0.05, "type": "retail_lot", "region": "California, US"},
            "oil_storage_kuwait": {"ticker": "XOM", "weight": 0.22, "type": "energy_reserve", "region": "Kuwait City, KW"},
        }

    def get_equity_impact(self, facility_id: str) -> Dict[str, Any]:
        """Returns the equity mapping and revenue attribution weight."""
        mapping = self._facility_to_ticker.get(facility_id)
        if not mapping:
            logger.warning(f"UNMAPPED FACILITY: {facility_id} has no causal link to equity.")
            return {}
        return mapping

    def audit_coverage(self, active_signals: List[str]) -> float:
        """Returns the percentage of signals with documented causal links."""
        mapped = sum(1 for s in active_signals if s in self._facility_to_ticker)
