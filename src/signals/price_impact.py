...
class ICAnalyzer:
    ...
    def compute_ic_series(
        ...
        naive_momentum: Optional[np.ndarray] = None, # No-signal baseline comparison
        vix_series: Optional[np.ndarray] = None, # VIX conditional macro regression blocks
    ) -> dict[str, Any]:
        ...
        if signals.ndim == 1:
            ...
            if vix_series is not None:
                ...
                if np.sum(high_vix) > 5 and np.sum(~high_vix) > 5:
                    ic_high, _ = stats.spearmanr(signals[valid][high_vix], forward_returns[valid][high_vix])
                    ic_low, _ = stats.spearmanr(signals[valid][~high_vix], forward_returns[valid][~high_vix])
    ...
    def fit(
        ...
        try:
            ...
            self._model = lgb.train(
                {k: v for k, v in self._params.items() if k not in ("n_estimators", "early_stopping_rounds")},
                train_data,
