...
class RampManager:
    ...
    def record_fill_quality(self, symbol: str, expected: float, actual: float):
        ...
        if abs(slippage) > 15.0: # 15bps threshold
            logger.warning(f"HIGH SLIPPAGE: {symbol} | Predicted: {expected} | Actual: {actual} | Error: {slippage:.1f}bps")

    def get_aggregate_slippage_error(self) -> float:
        """Average slippage error in bps across all fills."""
        if not self.fill_history: return 0.0
    ...
    def promote_ramp(self):
        ...
        if self.current_ramp_pct == 0.10: self.current_ramp_pct = 0.25
        elif self.current_ramp_pct == 0.25: self.current_ramp_pct = 0.50
        elif self.current_ramp_pct == 0.50: self.current_ramp_pct = 1.00
