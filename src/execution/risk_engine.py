...
class Portfolio:
    ...
    def sector_exposure(self, sector: str) -> float:
        return sum(abs(p.notional_usd) for p in self.positions if p.sector == sector)

    def country_exposure(self, country: str) -> float:
        return sum(abs(p.notional_usd) for p in self.positions if p.country == country)
...
class RiskEngine:
    ...
    def _connect(self) -> sqlite3.Connection:
        if self._conn is not None:
            return self._conn
        return sqlite3.connect(self.audit_db)
    ...
    def gate_liquidity(self, order: Order, _: Portfolio) -> GateResult:
        ...
        if participation > 0.05:
            return GateResult(
                "FAIL", "liquidity", f"Order is {participation * 100:.1f}% of ADTV. Max: 5%."
            )
        return GateResult("PASS", "liquidity")

    def gate_single_name(self, order: Order, portfolio: Portfolio) -> GateResult:
        current = sum(abs(p.notional_usd) for p in portfolio.positions if p.ticker == order.ticker)
    ...
    def gate_var_parametric(self, _order: Order, portfolio: Portfolio) -> GateResult:
        ...
        if estimated_var > VAR_99_LIMIT:
            return GateResult(
                "FAIL",
                "var_parametric",
                f"Est. VaR 99% = {estimated_var * 100:.2f}% NAV. Max: {VAR_99_LIMIT * 100:.1f}%.",
            )
    ...
    def gate_compliance(self, order: Order, _: Portfolio) -> GateResult:
        ...
        if order.ticker in BLOCKED_TICKERS:
            return GateResult(
                "FAIL", "compliance", f"{order.ticker} is on the compliance blocklist."
            )
    ...
    def authorize_kill(self, kill_request_id: str, authorizing_operator_id: str) -> KillResult:
