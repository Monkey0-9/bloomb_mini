...
class CostModel:
    def __init__(
        ...
        if any(v == 0.0 for v in bid_ask_bps_by_mcap_tier.values()) and commission_bps == 0.0:
            raise ValueError("Zero-cost backtest is rejected. Institutional trading always has costs.")
        self.bid_ask = bid_ask_bps_by_mcap_tier
        self.comm = commission_bps

    def compute_impact_bps(self, order_size_usd: float, adtv_usd: float, sigma_daily: float) -> float:
...
class BacktestEngine:
    ...
    def compute_sharpe_with_ci(
        ...
        for _ in range(n_bootstrap):
            ...
            indices = np.random.choice(n - block_size_weeks, size=n // block_size_weeks, replace=True)
    ...
    def run_backtest(self, returns_series: pd.Series) -> BacktestResult:
        ...
        res = BacktestResult(
            ...
            max_drawdown=float(returns_series.cumsum().max() - returns_series.cumsum().min()), # Simplified
