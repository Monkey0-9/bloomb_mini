...
async def run_squawk_pipeline(update_callback):
    ...
    while True:
        try:
            ...
            for al in alerts:
                # Deduplicate so we don't spam the frontend every 15s for the same emergency
                incident_key = f"{al['id']}-{al['squawk']}"
                if incident_key not in seen_alerts:
                    print(f"[SQUAWK] 🚨 EMERGENCY DETECTED: {al['callsign']} SQUAWK {al['squawk']} ({al['type']})")
