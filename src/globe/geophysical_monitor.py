...
class GeophysicalEvent:
    ...
    lat: float
    lon: float
    magnitude: float | None  # For earthquakes
...
class GeophysicalMonitor:
    ...
    def process_earthquake(self, eq: dict[str, Any]) -> GeophysicalEvent | None:
        ...
        return GeophysicalEvent(
            ...
            timestamp=eq.get('time', datetime.now(UTC)),
    ...
    def process_conflict(self, conflict: dict[str, Any]) -> GeophysicalEvent | None:
        ...
        return GeophysicalEvent(
            ...
            timestamp=datetime.strptime(conflict.get('date', ''), '%Y-%m-%d').replace(tzinfo=UTC) if conflict.get('date') else datetime.now(UTC),
            affected_chokepoints=[c['name'] for c in nearby],
            affected_tickers=all_tickers,
...
# Singleton instance
_monitor: GeophysicalMonitor | None = None
