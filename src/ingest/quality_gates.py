...
def gate_cloud_cover(m: TileMetadata) -> QualityGateResult:
    if m.cloud_cover_pct is None:
        return QualityGateResult(
            "cloud_cover", "REJECT", m.tile_id, "cloud_cover_pct is null — cannot assess quality"
        )
    ...
    return QualityGateResult("cloud_cover", "PASS", m.tile_id, metric_value=m.cloud_cover_pct)
...
def gate_schema(m: TileMetadata) -> QualityGateResult:
    ...
    for field_name, value in required.items():
        if not value:
            return QualityGateResult(
                "schema", "REJECT", m.tile_id, f"Required field '{field_name}' is empty or null"
            )
    return QualityGateResult("schema", "PASS", m.tile_id)


def gate_checksum(tile_id: str, file_path: str, stored_checksum: str) -> QualityGateResult:
    if not Path(file_path).exists():
        return QualityGateResult("checksum", "REJECT", tile_id, f"File not found: {file_path}")
    h = hashlib.sha256()
...
def gate_license(m: TileMetadata, catalog_db: str = "data/catalog.db") -> QualityGateResult:
    if not m.commercial_use_ok:
        with sqlite3.connect(catalog_db) as conn:
            ...
            conn.commit()
        return QualityGateResult(
            "license", "BLOCK", m.tile_id, f"License {m.license_id}: commercial use not permitted"
        )
...
def gate_duplicate(m: TileMetadata, catalog_db: str = "data/catalog.db") -> QualityGateResult:
    try:
        ...
        if row:
            return QualityGateResult(
                "duplicate", "REJECT", m.tile_id, f"Duplicate of existing tile {row[0]}"
            )
...
def gate_geolocation(m: TileMetadata) -> QualityGateResult:
    if len(m.bbox_wgs84) != 4:
        return QualityGateResult(
            "geolocation", "REJECT", m.tile_id, "bbox_wgs84 must have exactly 4 values"
        )
    min_lon, min_lat, max_lon, max_lat = m.bbox_wgs84
    if not (-180 <= min_lon <= 180 and -180 <= max_lon <= 180):
        return QualityGateResult(
            "geolocation", "REJECT", m.tile_id, f"Longitude out of range: {min_lon}, {max_lon}"
        )
    if not (-90 <= min_lat <= 90 and -90 <= max_lat <= 90):
        return QualityGateResult(
            "geolocation", "REJECT", m.tile_id, f"Latitude out of range: {min_lat}, {max_lat}"
        )
