...
class GlobalIntelligenceEngine:
    ...
    def reverse_geocode(self, lat: float, lon: float) -> dict[str, str]:
        ...
        """
        try:
            # Cast params to dict[str, str | int] to satisfy requests
    ...
    def get_news_intelligence(self, ticker: str | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if ticker:
                rss_articles = aggregator.fetch_ticker_news(ticker, max_articles=20)
            else:
                ...
                for src_articles in multi.values():
                    rss_articles.extend(src_articles)

            sentiment = aggregator.get_market_sentiment(rss_articles)

            intelligence['articles'].extend([a.to_dict() for a in rss_articles])
            intelligence['sentiment'] = sentiment.get('sentiment', 'neutral')
            intelligence['sources'].append('rss')

            logger.info(f"RSS intelligence: {len(rss_articles)} articles, {sentiment['sentiment']} sentiment")
        ...
        try:
            ...
            if ticker:
                gdelt_articles = scraper.search_ticker_news(ticker, ticker, timespan="24h")
            else:
                gdelt_articles = scraper.search_maritime_intelligence(timespan="24h")

            impact = scraper.analyze_news_impact(gdelt_articles)

            # Merge GDELT articles
            gdelt_dicts = [a.to_dict() for a in gdelt_articles[:10]]
            ...
            elif urgency == 'medium' and intelligence['urgency'] == 'low':
                intelligence['urgency'] = 'medium'

            logger.info(f"GDELT intelligence: {len(gdelt_articles)} articles, urgency={impact.get('urgency')}")
    ...
    def _build_enhanced_reason(
        self, facility: str, sigma: float, frp: float,
        ...
        reason += "Likely earnings impact within 41 days."

        return reason


# Singleton instance
_engine: GlobalIntelligenceEngine | None = None
