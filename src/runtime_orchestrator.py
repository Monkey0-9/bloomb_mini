...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("daemon_runtime.log"), logging.StreamHandler(sys.stdout)],
)
...
def load_secrets() -> Any:
    ...
    if not secrets_path.exists():
        logger.error(
            "config/secrets.json not found! Please copy config/secrets.template.json and fill it."
        )
        sys.exit(1)
...
def run_iteration() -> None:
    ...
    try:
        ...
        logger.info(f"--- ITERATION COMPLETED SUCCESSFULLY AT {datetime.now(UTC)} ---")
    ...
    while True:
        try:
            ...
            if sleep_time > 0:
                ...
                logger.warning(
                    "Iteration took longer than the interval! Running next iteration immediately."
                )
